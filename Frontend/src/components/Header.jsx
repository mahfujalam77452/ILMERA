      ...
      <div className="bg-white shadow-lg rounded-xl w-full max-w-7xl pointer-events-auto relative">
        <div className="flex justify-between items-center h-16 sm:h-20 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/">
              <img src={logo} alt="Foundation Logo" className="h-14 sm:h-20 w-auto" />
            </Link>
          </div>
          ...
          <nav className="hidden xl:flex items-center space-x-6">
            ...
            <Link
              ...
              className={`text-sm font-bold transition-colors hover:text-[#0B4D26] ${
                isActive('/') ? 'text-[#0B4D26] border-b-2 border-[#0B4D26]' : 'text-gray-700'
              }`}
            >
              {t('menu.home')}
            </Link>
            ...
            <div className="group relative h-full flex items-center">
              <button className={`flex items-center gap-1 text-sm font-bold transition-colors hover:text-[#0B4D26] ${
                 location.pathname.includes('/appeals') ? 'text-[#0B4D26]' : 'text-gray-700'
              }`}>
                {t('menu.appeals')} 
            ...
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                className={`text-sm font-bold transition-colors hover:text-[#0B4D26] ${
                  isActive(link.path) ? 'text-[#0B4D26] border-b-2 border-[#0B4D26]' : 'text-gray-700'
                }`}
              >
                {link.name}
              </Link>
          ...
          <div className="hidden xl:flex items-center space-x-4">
            <div className="flex items-center bg-gray-100 rounded-md p-1">
              <button onClick={() => changeLanguage('bn')} className={`px-3 py-1 text-sm rounded-md transition-all ${i18n.language === 'bn' ? 'bg-white shadow text-[#0B4D26] font-bold' : 'text-gray-500 hover:text-gray-700'}`}>বা</button>
              <button onClick={() => changeLanguage('en')} className={`px-3 py-1 text-sm rounded-md transition-all ${i18n.language === 'en' ? 'bg-white shadow text-[#0B4D26] font-bold' : 'text-gray-500 hover:text-gray-700'}`}>EN</button>
            </div>
            ...
            <Link to="/donation" className="bg-[#0B4D26] hover:bg-green-800 text-white px-6 py-2 rounded-md font-medium transition-colors">
              {t('menu.donate')}
            </Link>
          </div>
          ...
          <div className="xl:hidden flex items-center space-x-3">
            <Link to="/donation" className="bg-[#0B4D26] text-white px-4 py-1.5 rounded-md text-sm font-medium">
              {t('menu.donate')}
            </Link>
            ...
            <div className="px-4 pt-2 pb-6 space-y-1">
              ...
              <div>
                ...
                >
                  ...
                  <svg className={`w-4 h-4 transition-transform ${isMobileAppealsOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"/>
                  </svg>
                </button>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between px-3">
                ...
                <div className="flex space-x-2">
                  <button onClick={() => changeLanguage('bn')} className={`px-4 py-1.5 border rounded-md font-bold ${i18n.language === 'bn' ? 'bg-green-50 border-[#0B4D26] text-[#0B4D26]' : 'text-gray-500'}`}>বা</button>
                  <button onClick={() => changeLanguage('en')} className={`px-4 py-1.5 border rounded-md font-bold ${i18n.language === 'en' ? 'bg-green-50 border-[#0B4D26] text-[#0B4D26]' : 'text-gray-500'}`}>EN</button>
                </div>
              </div>
            </div>
          </div>
